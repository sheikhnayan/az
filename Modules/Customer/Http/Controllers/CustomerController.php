...
class CustomerController extends Controller
{
    ...
    public function customer_affiliate()
    {
        ...
        return view('customer::customers.affiliate',compact('data'));
    }
    ...
    public function updateInfo(Request $request)
    {
        ...
        if (auth()->user()->phone) {
            $phone = 'required|min:7|max:16|unique:users,phone,'.auth()->user()->id;
        }else{
            $phone = 'nullable|min:7|max:16|unique:users,username,'.auth()->user()->id;
        }
        $request->validate([
            'first_name' => 'required',
            'email' => $email,
            ...
            "phone.min" => "Minimum 7 digits required on phone number"
        ]);
        try {
